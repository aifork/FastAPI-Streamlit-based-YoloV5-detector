...
async def root(file: UploadFile = File(...)):
    ...
    im=cv2.putText(im,"persons : {}".format(len(list(a for a in results.pandas().xyxy[0]['name']=='person' if a!= False))),(150,200),cv2.FONT_HERSHEY_SIMPLEX,9,(255,0,0),7)
...
async def vid(file: UploadFile = File(...)):
    ...
    for i in range(count):
        ...
        im=cv2.putText(im,"persons : {}".format(len(list(a for a in results.pandas().xyxy[0]['name']=='person' if a!= False))),(100,100),cv2.FONT_HERSHEY_SIMPLEX,4,(255,0,0),4)
        cv2.imwrite('C:/Users/BHASKAR BOSE/runs/detect/frame{}.jpg'.format(i),im)
    img_array = []
#for filename in glob.glob('C:/Users/BHASKAR BOSE/runs/detect/*.jpg'):
    for i in range(count):
    ...
    im=cv2.imread('C:/Users/BHASKAR BOSE/runs/detect/exp/{}'.format(file.filename))
    im=cv2.putText(im,"persons : {}".format(len(list(a for a in results.pandas().xyxy[0]['name']=='person' if a!= False))),(150,200),cv2.FONT_HERSHEY_SIMPLEX,9,(255,0,0),7)
    cv2.imwrite('C:/Users/BHASKAR BOSE/runs/detect/{}'.format(file.filename),im)
    """
